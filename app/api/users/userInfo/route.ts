...
export async function GET(): Promise<NextResponse<UserInfoResponse>> {
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get('KALALOG_TOKEN')?.value;
    if (!token) {
      return NextResponse.json<UserInfoResponse>(
        { message: 'User not logged in', data: { loggedIn: false, jwtUserInfo: null } },
        { status: 200 }
      );
    }
    ...
    return NextResponse.json<UserInfoResponse>(
      { message: 'User info fetched succesfully', data: { loggedIn: true, jwtUserInfo: decoded } },
      { status: 200 }
    );
  } catch (error) {
