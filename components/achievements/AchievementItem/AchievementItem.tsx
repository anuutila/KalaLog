...
import { IconLock, IconStarFilled, IconTrophy } from '@tabler/icons-react';
...
function getStarInfo(achievementConfig: IAchievementConfig, userAchievement?: IAchievement): StarInfo {
...
function getColors(achievementConfig: IAchievementConfig, { isOneTime, filledStars, isUnlocked }: StarInfo) {
  const achievementConfigOneTime = achievementConfig as IAchievementConfigOneTime;
  ...
  if (!isUnlocked) {
    ...
    isUnlocked &&
    ((isOneTime && achievementConfigOneTime.rarity === 1) || (!isOneTime && filledStars === 1))
  ) {
    ...
    if (
      ...
      return (
        <Group gap="sm" wrap="nowrap">
          {isUnlocked && (
            <Text fz={12} lh={1} fw="bold" className={classes.nowrap}>
              {(userAchievement as IAchievementTiered).currentTier < baseTiersCount
                ? t('AchievementsPage.NextTier')
                : ''}
            </Text>
          )}
      ...
      achievementConfig.progressBar !== false &&
      filledStars >= baseTiersCount &&
      !tA.has(`${achievementConfig.key}.Progress`)
    ) {
      return (
        <Group gap="sm" wrap="nowrap">
          <Text fz={12} lh={1} fw="bold" className={classes.nowrap}>
            ...
            <Group align="center" gap={4}>
              {Array.from({ length: starInfo.totalStars }).map((_, idx) => (
                <IconStarFilled
                  key={idx}
                  size={20}
                  color={idx < starInfo.filledStars ? starColor : 'var(--mantine-color-dark-4)'}
                />
              ))}
            </Group>
          </Group>
