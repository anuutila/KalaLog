...
import { IPopulatedEventUserSchema } from '@/lib/types/event';
import { getSecondaryMetricIcon } from '@/components/charts/LeaderboardBarChart/LeaderboardBarChart';
...
const yAxisLabelPlugin: Plugin<'bar', YAxisLabelPluginOptions> = {
  ...
  afterDraw: (chart, args, options) => {
    ...
    yAxis.getTicks().forEach((tick, index) => {
      ...
      ctx.font = `${currentWeight} ${fontSize}px ${fontFamily}`;
      ctx.textAlign = 'left';
      const displayName = ((label?.length ?? 0) > 9 && user?.score) ? label?.substring(0, 6) + '...' : (label ? label : '');
...
function processGenericLeaderboardData(
  allUserInfos: IPopulatedEventUserSchema[],
  unregisteredParticipants: string[] | undefined,
  catches: ICatch[],
  ...
  switch (primaryMetric) {
    ...
    case 'perch':
      ...
      // Include all event participants (registered and unregistered, with or without score) in the leaderboard
      for (const user of [...allUserInfos ?? [], ...unregisteredParticipants ?? []]) {
        const userId = (user as IPopulatedEventUserSchema)?.id;
        const name = user;
        // Prefer userId as key if available, otherwise use name (for unregistered)
        const userKey = userId || `unregistered_${name}`;
...
function formatGenericChartJsData(
  ...
  // const targetLength = Math.min(rankedUsers.length, 10);
...
function generateChartOptions(
  ...
  const isStacked = ['totalCatches'].includes(primaryMetric);
  ...
  return {
    ...
    layout: {
      padding: {
        right: 35, // Increased right padding for datalabels
        left: 95
      }
    },
    plugins: {
...
type PrimaryMetric = 'totalCatches' | 'pike' | 'zander' | 'perch';
type SecondaryMetric = 'count' | 'weight' | 'length';

const primaryMetricKeys: PrimaryMetric[] = [
  'totalCatches',
  'pike',
  'zander',
  'perch'
...
export default function EventLeaderboardBarChart({ catches, userInfo, userDisplayNameMap, allUserInfos, unregisteredParticipants }: LeaderboardBarChartProps) {
