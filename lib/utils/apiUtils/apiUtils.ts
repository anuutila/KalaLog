...
): Promise<{ count: number; linkedName: string }> => {
  await dbConnect();

  try {
    const unlinkedCatches = await Catch.find({
      $or: [{ 'caughtBy.userId': { $exists: false } }, { 'caughtBy.userId': null }],
    });
    ...
    console.log(
      `Found ${catchesAlreadyLinkedToUser.length} catches already linked to user ${user.firstName} (${user.username})`
    );
    ...
    if (catchesToLink.length > 0) {
      const updateResult = await Catch.updateMany(
        ...
        {
          $set: {
            'caughtBy.name': user.firstName,
            'caughtBy.userId': user.id,
            'caughtBy.username': user.username,
            'caughtBy.lastName': user.lastName,
          },
        }
      );
